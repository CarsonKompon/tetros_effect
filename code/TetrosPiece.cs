...
public class TetrosPiece : Component
{
	...
	protected override void OnUpdate()
	{
		...
		Container.WorldRotation = Rotation.Lerp( Container.WorldRotation, Rotation.From( new Angles( customAngle, 0, 0 ) ), lerp );

		// Lerp to position
		lerp = 1f - MathF.Pow( 0.5f, Time.Delta * 50f );
		...
		LocalPosition = Vector3.Lerp( LocalPosition, targetPosition, lerp );
	}
	...
	public void SetPosition( Vector2 pos, bool instant = false )
	{
		...
		if ( instant )
		{
			LocalPosition = Board.GetLocalPosition( (int)Position.x, (int)Position.y ) + new Vector3( Board.GridSize / 2f, 0, -Board.GridSize / 2f );
		}
	}
	...
	public void SetRotation( int i, bool instant = false )
	{
		...
		if ( instant )
		{
			...
			Container.WorldRotation = Rotation.From( new Angles( customAngle, 0, 0 ) );
		}
	}
}
